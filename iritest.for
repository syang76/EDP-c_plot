...
C
      SUBROUTINE IRITEST_MAIN(
     &   jm,xlat,xlon,iy,imd,iut,hour,hx,ivar,vbeg,vend,vstp,
...
c        read(5,*) jm,xlat,xlon
        print *, 'Input jm,xlat,xlon=',jm,xlat,xlon
        print *,'year(yyyy),mmdd(or -ddd),iut(=0/1,LT/UT),hour'
c        read(5,*) iy,imd,iut,hour
        print *, 'Input iy,imd,iut,hour=', iy,imd,iut,hour
        print *,'height/km'
c        read(5,*) hx
        print *, 'Input hx=', hx
...
c        read(5,*) icontinue
        ...
        if (icontinue.eq.0) then
           print *, 'iri_test is completed. return to c_plot()'
           return
        endif
